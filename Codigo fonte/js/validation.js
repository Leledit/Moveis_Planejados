...
(function () {
  document.forms.enviar.noValidate = true; // Disable HTML5 validation - using JavaScript instead
  ...
  function validateRequired(el) {
    if (isRequired(el)) {                           // Is this element required?
      ...
      if (!valid) {                                 // If valid variable holds false
        setErrorMessage(el,'Campo obrigatorio');  // Set the error message
      }
  ...
  var validateType = {
    email: function (el) {                                 // Create email() method
      ...
      if (!valid) {                                        // If the value of valid is not true
        setErrorMessage(el, 'Entre com um E-mail valido'); // Set error message
      }
